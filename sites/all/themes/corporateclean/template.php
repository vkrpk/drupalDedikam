...
function corporateclean_preprocess_html(&$variables) {
	...
	drupal_add_css('//fonts.googleapis.com/css?family=Lato:300,400,700,300italic,400italic', array('group' => CSS_THEME));
...
function corporateclean_form_alter(&$form, &$form_state, $form_id) {
  ...
  if ( TRUE === in_array( $form_id, array( 'user_login', 'user_login_block') ) ){
	...
	if(isset($form['links']['#markup'])){
		$form['links']['#markup'] = '<div class="dedikam-login-link">'.$form['links']['#markup'].'</div>';
	}
}
 ...
 */
drupal_add_js(drupal_get_path('theme', 'corporateclean') . '/js/jquery.flexslider-min.js');
